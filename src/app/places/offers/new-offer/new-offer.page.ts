  ...
  onCreateOffer() {
    ...
    this.loadingCtrl.create({keyboardClose: true, message: "Creating offer"}).then(loadingEl => {
      ...
      this.placesService.uploadImage(this.form.get('image').value).pipe(switchMap(uploadResponse => {
        return this.placesService.addPlace(this.form.value.title, this.form.value.description, +this.form.value.price, new Date(this.form.value.dateFrom), new Date(this.form.value.dateTo), this.form.value.location, uploadResponse.imageUrl);
      }))
      .subscribe(() => {
        loadingEl.dismiss();
